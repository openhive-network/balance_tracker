...
BEGIN
  ...
  -- 4) DELETE fills, but only the asset side that was actually paid
  WITH fills_info AS (
    SELECT
  ...
  -- 8) Final diagnostics
  RAISE NOTICE
    'Blocks %–% | new_creates=% | cancels_deleted=% | fills_deleted=% | total_closed=% | summary_upserts=% | appended_ids=% | purged=%',
