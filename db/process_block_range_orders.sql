...
BEGIN
  --------------------------------------------------------------------------------
  -- 0) Silently drop any previous temp table (no NOTICE if missing)
  --------------------------------------------------------------------------------
  BEGIN
    DROP TABLE tmp_raw_ops_flat;
  EXCEPTION WHEN undefined_table THEN
    NULL;  -- ignore if it wasn’t there
  END;

  --------------------------------------------------------------------------------
  -- 1) Pull raw ops for this slice
  --------------------------------------------------------------------------------
  CREATE TEMP TABLE tmp_raw_ops_flat ON COMMIT DROP AS
  SELECT
    ov.block_num,
    ot.name        AS op_name,
    ov.body::jsonb AS j
