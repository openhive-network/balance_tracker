 ...
 *
 * The operation type ID (op_type_id) is stored directly in each base table,
 * populated during processing from the operations_view. This eliminates the
 * need for JOINs to hafd.operations at query time.
 *
 * Extracting block numbers still requires the HAF function
 * hafd.operation_id_to_block_num(). By defining views, we compute this
 * derived value once and reuse it across many queries.
 *
 * HAF Functions Used:
 * -------------------
 ...
 *
 * Columns Exposed from Base Tables:
 * ---------------------------------
 ...
 * Since op_type_id is stored in the base tables, no JOIN to hafd.operations
 * is needed, which significantly improves query performance.
 *
