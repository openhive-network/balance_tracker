...
-- Single-row aggregate shape per blockchain operation
...
-- Single-row parser; excludes approve_intent; dispute omitted
...
BEGIN
  ...
  ELSIF _op_name = 'hive::protocol::escrow_release_operation' THEN
    ...
    r.hbd_amount  := NULLIF((_body->'value'->'hbd_amount' ->>'amount'), '')::bigint;
    r.hive_amount := NULLIF((_body->'value'->'hive_amount'->>'amount'), '')::bigint;
    r.fee_amount := NULL;  -- no fee on release
    r.fee_nai    := NULL;

  ELSIF _op_name = 'hive::protocol::escrow_approved_operation' THEN
    r.kind       := 'approved';
    r.fee_amount := NULLIF((_body->'value'->'fee'->>'amount'), '')::bigint;        -- approval fee (if present)
    r.fee_nai    := NULLIF(substring((_body->'value'->'fee'->>'nai') FROM 3), '')::smallint;
    r.hbd_amount := NULL;
    r.hive_amount:= NULL;
  ...
  ELSE
    r.kind := NULL; -- ignore unknown/unneeded kinds (e.g., approve_intent)
  END IF;
