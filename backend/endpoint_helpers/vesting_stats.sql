...
per-account pre-aggregated tables (account_vesting_by_day /
_by_month) populated during sync by process_account_vesting_stats.

No on-the-fly scans of hive.account_operations_view or
account_vesting_history at query time — everything served from
pre-aggregated tables, with yearly rolled up from monthly via
btracker_backend.account_vesting_stats_by_year.

Mirrors btracker_backend.get_vesting_aggregation (global) in shape and
gap-fill behaviour.
*/
DO $$
DECLARE
