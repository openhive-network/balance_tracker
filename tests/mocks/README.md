...
Operations are inserted with IDs generated by `hafd.operation_id(block_num, op_pos)`. The `op_type_id` is stored as a separate column. This matches HAF's internal ID scheme and ensures operations can be looked up correctly.
